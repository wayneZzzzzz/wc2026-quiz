...
async function initDb(SqlLib) {
  ...
  const ROUND_OF_32_SEED = hasExistingMatches ? [
    ...
    { home:'墨西哥', away:'厄瓜多尔', time:'2026-07-01 01:00',
      desc:'墨西哥让0.5球', a:'墨西哥赢球', b:'平局', c:'厄瓜多尔赢球' },
    { home:'英格兰', away:'刚果', time:'2026-07-01 16:00',
      desc:'英格兰让1.5球', a:'英格兰赢2球及以上', b:'英格兰赢1球', c:'平局或刚果赢球' },
    { home:'比利时', away:'塞内加尔', time:'2026-07-01 20:00',
      desc:'比利时让0.5球', a:'比利时赢球', b:'平局', c:'塞内加尔赢球' },
    { home:'西班牙', away:'奥地利', time:'2026-07-02 19:00',
      desc:'西班牙让1.5球', a:'西班牙赢2球及以上', b:'西班牙赢1球', c:'平局或奥地利赢球' },
    { home:'葡萄牙', away:'克罗地亚', time:'2026-07-02 23:00',
      desc:'葡萄牙让0.5球', a:'葡萄牙赢球', b:'平局', c:'克罗地亚赢球' },
    { home:'瑞士', away:'阿尔及利亚', time:'2026-07-03 03:00',
      desc:'瑞士让0.5球', a:'瑞士赢球', b:'平局', c:'阿尔及利亚赢球' },
    { home:'澳大利亚', away:'埃及', time:'2026-07-03 18:00',
      desc:'平手盘', a:'澳大利亚赢球', b:'平局', c:'埃及赢球' },
    { home:'哥伦比亚', away:'加纳', time:'2026-07-04 01:30',
      desc:'哥伦比亚让1球', a:'哥伦比亚赢2球及以上', b:'哥伦比亚赢1球', c:'平局或加纳赢球' },
  ] : [];
  ...
  const QUARTER_FINAL_SEED = hasExistingMatches ? [
    ...
    { home:'阿根廷', away:'瑞士', time:'2026-07-12 01:00',
      desc:'阿根廷让1球', a:'阿根廷赢2球及以上', b:'阿根廷赢1球', c:'平局或瑞士赢球' },
  ] : [];
  ...
  for (const m of FINAL_STAGE_SEED) {
    const exists = sqlDb.exec(`SELECT id FROM matches WHERE home_team='${m.home}' AND away_team='${m.away}' AND stage='${m.stage}'`);
    if (exists.length === 0 || exists[0].values.length === 0) {
      sqlDb.run(
        `INSERT INTO matches (home_team, away_team, match_time, stage, handicap_desc, option_a, option_b, option_c, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'upcoming')`,
        [m.home, m.away, m.time, m.stage, m.desc, m.a, m.b, m.c]
      );
    }
  }
